    ...
    begin
     if (rst)
      ...
      // ------------LEFT PADDLE-----------------
       ...
       lm <= lm + (vy_l * 6);   //update the position of left paddle
       rm <= rm + (vy_r * 6) ;   //update the position of right paddle
      end
    end   
...
wire hit_right_paddle = (h + R < 616)
                     && (h_next + R >= 616)
                     && (k_next >= c - (speed_y * 5)) && (k_next <= d + (speed_y * 5));
                     
wire hit_left_paddle  = (h - R > 23)
                     && (h_next - R <= 23)
                     && (k_next >= a - (speed_y * 5)) && (k_next <= b + (speed_y * 5));
