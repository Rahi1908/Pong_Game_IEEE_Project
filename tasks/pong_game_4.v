...
module pong_game_4 (                 // CHENGED FSM , NEED TO CHECK WHETHER WORKS OR NOT ..........YAYYYYYY WORKING!!
input clk,
input rst,
...
wire hit_right_paddle = (h + R < 616)  &&  // was outside last cycle
                        (h_next + R >= 616) && 
                        (k_next >= c && k_next <= d);

wire hit_left_paddle  = (h - R > 23)   &&  // was outside last cycle
                        (h_next - R <= 23) && 
                        (k_next >= a && k_next <= b);
...
always @(*) begin
    ...
    if (game_active) begin
        case(curr_state)
            IDLE: if (lu || ld || ru || rd)            nxt_state = PLAY;
            PLAY: if (hit_right_wall || hit_left_wall) nxt_state = MISS;
            MISS:                                      nxt_state = IDLE;
...
// ------------------ sequential block -------------------------
always @(posedge clk_120HZ or posedge rst) begin
    if (rst) begin
        curr_state        <= IDLE;
        h                 <= 17'sd320;
        k                 <= 17'sd240;
        ...
        if (game_active) begin
            case(curr_state)
                ...
                PLAY: begin
                    ...
                    if (hit_right_paddle || hit_left_paddle)
                        dx <= -dx;
        ...
        // game over delay — runs on clk_120HZ, 120 cycles = 1 second
        if ((p1_score == 2'd3 || p2_score == 2'd3) && !game_over_flag) begin
            if (delay_cnt < 27'd120)
