...
// ── AI velocity wire for left paddle ──────────────────────────
// AI only reacts when ball is coming toward it (dx < 0)
// hard_mode: reacts from anywhere on court
// easy_mode: only reacts when ball is in left half
wire signed [16:0] lm_vel_ai =
    (dx < 0 && ((hard_mode && h < 17'sd615) || (!hard_mode && h < 17'sd320))) ?
        ((k < lm) ? -PADDLE_SPEED :
         (k > lm) ?  PADDLE_SPEED : 17'sd0)
    : 17'sd0;

// ── select velocity based on mode ─────────────────────────────
wire signed [16:0] lm_vel = AI_mode ? lm_vel_ai : lm_vel_manual;
wire signed [16:0] rm_vel = rm_vel_manual;   // right always manual
...
always @ (posedge clk_60HZ or posedge rst) begin
    if (rst) begin
        ...
        // LEFT - clamp handles top wall, bottom wall, and powerup growth
    ...
    // when game_start=1, paddles hold their position (no else needed)
end
