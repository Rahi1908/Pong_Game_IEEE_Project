...
module pong_game(            //THIS MODULE IS UNDER BEBUGGING ,STILL NOT GETTING WHAT IS WRONG HERE
input clk,
input rst,
...
output [7:0] an
    ...
    parameter R = 17'sd16;  // Radius of ball 16 px
    parameter INT = 3'b000, S1 = 3'b001, S2 = 3'b010, S3 = 3'b011, S4 = 3'b100, S0 = 3'b101 ; //Finite states
    ...
    reg [1:0] p2_score;
    wire clk_500HZ;
    wire signed [16:0] rdm_speed;
    reg signed [16:0] speed_x = 17'sd1;
    reg signed [16:0] speed_y = 17'sd1;
...
//-------------------------------------------------------------------------------------
    ...
    score_display uut_score_dis ( .clk_500HZ(clk_500HZ), .rst(rst), .p1_score(p1_score), .p2_score(p2_score), .seg(seg), .an(an) );
    
    after_paddle_hit_vel vel_gen ( .clk(clk), .rst(rst), .speed_mag(rdm_speed) );
   ...
   wire signed [16:0] y;
   wire signed [34:0] z;
   assign a = lm - 17'd35;  //paddles height 70px
   assign b = lm + 17'd35;
   assign c = rm - 17'd35;
   assign d = rm + 17'd35;
...
if (h_count >= 356 && h_count <= 382 && v_count >= 24 && v_count <= 48) begin
    ...
    endcase
     ...
     if ( z <= 35'sd256 )
      begin
       red = 4'b1111;         // White Ball with radius 16
       blue = 4'b1111;  
       grn = 4'b1111;
      end
  ...
  wire hit_top_wall;
  wire hit_bottom_wall;
  reg paddle_hit_d;
  
  assign hit_right_paddle = (h_next + R >= 616) && (k_next >= c && k_next <= d);
  assign hit_left_paddle  = (h_next - R <= 23)  && (k_next >= a && k_next <= b);

  assign hit_top_wall    = (k_next - R <= 7);
  assign hit_bottom_wall = (k_next + R >= 472);
  
  always @ ( posedge clk_120HZ or posedge rst )   //Update state register
    begin
        if(rst)
          begin
             curr_state <= INT;
   ...
   always @ (posedge clk_120HZ or posedge rst) //Update scores of players
    ...
    //Edge detection
    always @(posedge clk_120HZ)
        begin
            paddle_hit_d <= (hit_left_paddle || hit_right_paddle);
        end

    wire paddle_hit_pulse = (hit_left_paddle || hit_right_paddle) && !paddle_hit_d;
    ...
    always @(posedge clk_120HZ)
        begin
            if (rst)
             ...
             // Wall hit → RESET SPEED
            else if (hit_top_wall || hit_bottom_wall)
                begin
                    speed_x <= 17'sd1;
                    speed_y <= 17'sd1;
                end
   ...
   always @ ( * )               //Compute the next state (next state decoder) 
    begin
     ...
     case(curr_state)
       ...
       S0: begin
            if( (dx_rdm == 17'sd1) && (dy_rdm == 17'sd1) ) nxt_state = S1;
            if( (dx_rdm == 17'sd1) && (dy_rdm == -17'sd1) ) nxt_state = S2;
            if( (dx_rdm == -17'sd1) && (dy_rdm == 17'sd1) ) nxt_state = S4;
            if( (dx_rdm == -17'sd1) && (dy_rdm == -17'sd1) ) nxt_state = S3;   
           end
    ...
    always @ (*)              //Compute the output
        begin
            ...
            case(curr_state)
                ...
                S0: begin
                        dx = dx_rdm; //random velocity direction
                        dy = dy_rdm;
                    end
